...
- `contracts/` Foundry: `HumanRegistry`, `PermissionRegistry`, `ValidationReceipts`, `PenaltyLedger`, `WorldIDVerifier`
  (the repo root currently holds these; task CC-0 moves them to `contracts/`)
...
- `docs/` RULES, TRUST, DEBRIEF, LIMITS, BUILD_PLAN

## Commands
- `cd contracts && forge build && forge test` (must stay green; currently 55 tests)
- `cd app && npm run lint && npm run typecheck && npm test`
- Local chain demo (legacy orchestrator): `cd orchestrator && node demo.js`
