...
| `src/PenaltyLedger.sol` | `f28c1ed07a90d21a` | verdict token, decaying score, stages, optional count ladder, judge lift |
...
| C-12 | The **judge** (`PenaltyLedger.JUDGE_ROLE`) may lift restrictions | Demo trust assumption: an address, not an enrolled human. `judgeLift` sets a score to 0 with a reason hash (token records stay, `Forgiven` event). It can never lift a count-based ban (`banAtCount`, `BannedForever`) and can't lift its own human. Granted only by `environments/demo.json` (to the relayer). The optional ladder (`setLadder`: `weightByCount`, `banAtCount`) is off by default; tests: `test/Ladder.t.sol`, `test_OtherPresetsHaveNoLadderAndNoJudge`. |
| C-13 | `ChallengeRewards`: the judge awards and slashes reward points; the owner funds, slashes and withdraws | Revision 4, new file `src/ChallengeRewards.sol` (tests: `test/Rewards.t.sol`). Points are soulbound (no transfer function), one per real standing receipt of the same validator, rate-limited by `cooldown`. Opt-in at `threshold` is final; after the deadline each opted-in human claims an equal share (pool snapshot at the first claim, `nonReentrant`, effects before the transfer, paid to `msg.sender` = the human's current account). The owner can withdraw only if nobody opted in. Trust: the judge can withhold points or slash anyone; the owner can slash. |
